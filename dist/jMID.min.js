...
subtype:"noteOn",deltaTime:0,time:a.time||0,velocity:a.velocity||90,noteNumber:a.noteNumber||32,track:a.track||null,channel:a.channel||1});this.noteOff=a.noteOff?a.noteOff:new d.Event({type:"channel",subtype:"noteOff",deltaTime:0,time:a.length?this.noteOn.time+a.length:this.noteOn.time+100,velocity:this.noteOn.velocity,noteNumber:this.noteOn.noteNumber,track:a.track||null,channel:a.channel||0});b.call(this);a.track&&(this.track=a.track,this.track.hasEvent(this.noteOn)||this.track.insertEventAtTime(this.noteOn),
this.track.hasEvent(this.noteOff)||this.track.insertEventAtTime(this.noteOff),this.track.insertNoteAtTime(this))};var b=function(){this.end=this.noteOff.time;this.start=this.noteOn.time;this.velocity=this.noteOn.velocity;this.noteNumber=this.noteOn.noteNumber;this.length=this.noteOff.time-this.noteOn.time};d.Note.prototype={adjustTime:function(a){this.track.adjustEventTime(this.noteOn,a);this.track.adjustEventTime(this.noteOff,a);this.track.positionNote(this);b.call(this)},adjustLength:function(a){this.track.adjustEventTime(this.noteOff,
...
h.getEvents();if(a)for(var r=0,m=m.length;r<m;r++)for(var o=this.noteSearch?h.getNotes(r):h.getEvents(r),p=!0,t=0,w=b.length;t<w;t++){for(var u=b[t],s=0,x=u.length;s<x;s++){var q;a:{q=u[s];for(var n=[":",">",">=","<","<="],k=n.length-1;0<=k;k--)if(-1!==q.search(n[k])){q=n[k];break a}q=void 0}k=u[s].split(q);n=k[0];k=k[1];if("number"==typeof value||toString.call(value)==numberClass)k=parseInt(k,10);switch(q){case ":":p=o[n]==k;break;case ">":p=o[n]>k;break;case ">=":p=o[n]>=k;break;case "<":p=o[n]<
k;break;case "<=":p=o[n]<=k}if(!p)break}p&&(this.noteSearch?c[e].pushNote(o):c[e].pushEvent(o))}else c[e]=h}return{tracks:c}},a=function(a,b,c){this.noteSearch=c;this._results=b?b:{tracks:a.tracks.slice(0)};this._file=a};a.prototype={filter:function(c){c=b.call(this,c);return new a(this._file,c,this.noteSearch)},not:function(c){for(var c=b.call(this,c),f=this._results,e=[],h=0,j=f.tracks.length;h<j;h++){var v=f.tracks[h],m=new d.Track;this.noteSearch?(m.notes=v.cloneNotes(),m.removeNotes.apply(m,
c.tracks[h].getNotes())):(m.events=v.cloneEvents(),m.removeEvents.apply(m,c.tracks[h].getEvents()));e.push(m)}return new a(this._file,{tracks:e},this.noteSearch)},toArray:function(){for(var a=[],b=0,c=this._results.tracks.length;b<c;b++)var d=this._results.tracks[b],a=a.concat(this.noteSearch?d.notes:d.events);return a},increment:function(a,b){d.Util[this.noteSearch?"forAllNotes":"forAllEvents"](this._results.tracks,function(c){a in c&&(c[a]+=b)});return this},set:function(a,b){d.Util[this.noteSearch?
"forAllNotes":"forAllEvents"](this._results.tracks,function(c){a in c&&(c[a]=b)});return this},get:function(a,b){return this._results.tracks[a].events[b]},eq:function(b,c){return new a(this._file,{tracks:[new d.Track({events:[this.get.apply(this,arguments)],timing:b.timing})]})},encodeEvents:function(a){var b=[];d.Util.forAllEvents(this._results.tracks,function(c,e,h){b[h]||(b[h]=[]);b[h].push(!a?d.Util.bytesToString(c.encode()):c.encode())});return b},encodeTracks:function(a){var b=[];d.Util.forEachTrack(this._results.tracks,
